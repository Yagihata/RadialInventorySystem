...
namespace YagihataItems.RadialInventorySystemV4
{
    ...
    public class Avatar
    {
        ...
        public void SaveToJson(string jsonPath)
        {
            ...
            {
                EditorUtility.DisplayDialog(RISStrings.GetString("ris"), "エラー：設定の保存に失敗しました", RISStrings.GetString("ok"));
            }
        }
        public static Avatar LoadFromJson(string path)
        {
            ...
            {
                EditorUtility.DisplayDialog(RISStrings.GetString("ris"), "エラー：設定の読み込みに失敗しました", RISStrings.GetString("ok"));
                return null;
            }
        }
        ...
        public void ReCreateExpressionMenu()
        {
            Debug.LogWarning($"Forced to recreate experession menu. => {UniqueID}");
            EditorTab editorTab = null;
            if (this.MenuMode == RIS.MenuModeType.Simple)
                editorTab = new TabSimple();
            else if (this.MenuMode == RIS.MenuModeType.Basic)
                editorTab = new TabBasic();
            else if (this.MenuMode == RIS.MenuModeType.Advanced)
                editorTab = new TabAdvanced();
            if (editorTab != null)
            {
                var autoGeneratedFolder = RIS.AutoGeneratedFolderPath + UniqueID + "/";
                if (!AssetDatabase.IsValidFolder(autoGeneratedFolder))
                    UnityUtils.CreateFolderRecursively(autoGeneratedFolder);
                var avatar = this;
                editorTab.BuildExpressionsMenu(ref avatar, autoGeneratedFolder);
            }
        }
        public void ReCreateExpressionParams()
        {
            Debug.LogWarning($"Forced to recreate experession menu. => {UniqueID}");
            EditorTab editorTab = null;
            if (this.MenuMode == RIS.MenuModeType.Simple)
            ...
            if (editorTab != null)
            {
                ...
                if (!AssetDatabase.IsValidFolder(autoGeneratedFolder))
                    UnityUtils.CreateFolderRecursively(autoGeneratedFolder);
                var avatar = this;
                editorTab.BuildExpressionParameters(ref avatar, autoGeneratedFolder);
            }
        }
    }
}
