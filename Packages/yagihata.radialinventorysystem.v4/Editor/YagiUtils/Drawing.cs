...
namespace YagihataItems.RadialInventorySystemV4
{
    public static class Drawing
    {
        ...
        private static Texture2D aaShortLineTex = null;
        ...
        public static void DrawLine(Vector2 pointA, Vector2 pointB, Color color, float width, bool antiAlias, float antiAliasLevel = 5f)
        {
            if (antiAlias)
            {
                var vect = (pointB - pointA).normalized;
                pointA -= vect * antiAliasLevel;
                pointB += vect * antiAliasLevel;
            }
            ...
            if (antiAlias)
            {
                ...
                tex = aaLineTex;
                mat = blendMaterial;
                if (Vector2.Distance(pointB, pointA) < 40)
                    tex = aaShortLineTex;
        ...
        private static void Initialize()
        {
            ...
            if (aaLineTex == null)
            {
                ...
                var width = 30;
                aaLineTex = new Texture2D(width, 3, TextureFormat.ARGB32, false);
                for (int i = 1; i < width - 1; i++)
                {
                    aaLineTex.SetPixel(i, 0, new Color(1, 1, 1, 0));
                    aaLineTex.SetPixel(i, 1, Color.white);
                    aaLineTex.SetPixel(i, 2, new Color(1, 1, 1, 0));
                }
                aaLineTex.SetPixel(0, 0, new Color(1, 1, 1, 0));
                aaLineTex.SetPixel(0, 1, new Color(1, 1, 1, 0));
                aaLineTex.SetPixel(0, 2, new Color(1, 1, 1, 0));

                aaLineTex.SetPixel(width - 1, 0, new Color(1, 1, 1, 0));
                aaLineTex.SetPixel(width - 1, 1, new Color(1, 1, 1, 0));
                aaLineTex.SetPixel(width - 1, 2, new Color(1, 1, 1, 0));
                aaLineTex.Apply();
            }
            if (aaShortLineTex == null)
            {
                // TODO: better anti-aliasing of wide lines with a larger texture? or use Graphics.DrawTexture with border settings
                var width = 5;
                aaShortLineTex = new Texture2D(width, 3, TextureFormat.ARGB32, false);
                for (int i = 1; i < width - 1; i++)
                {
                    aaShortLineTex.SetPixel(i, 0, new Color(1, 1, 1, 0));
                    aaShortLineTex.SetPixel(i, 1, Color.white);
                    aaShortLineTex.SetPixel(i, 2, new Color(1, 1, 1, 0));
                }
                aaShortLineTex.SetPixel(0, 0, new Color(1, 1, 1, 0));
                aaShortLineTex.SetPixel(0, 1, new Color(1, 1, 1, 0));
                aaShortLineTex.SetPixel(0, 2, new Color(1, 1, 1, 0));

                aaShortLineTex.SetPixel(width - 1, 0, new Color(1, 1, 1, 0));
                aaShortLineTex.SetPixel(width - 1, 1, new Color(1, 1, 1, 0));
                aaShortLineTex.SetPixel(width - 1, 2, new Color(1, 1, 1, 0));
                aaShortLineTex.Apply();
            }
