...
namespace YagihataItems.RadialInventorySystemV4
{
    public static class RISGimmickBuilder
    {
        ...
        public static void RemoveFromAvatar(RISVariables variables)
        {
            ...
            var autoGeneratedFolder = RIS.AutoGeneratedFolderPath + variables.FolderID + "/";
            ...
            if(variables.ApplyEnabled)
            {
                foreach(var group in variables.Groups)
                {
                    foreach(var prop in group.Props)
                    {
                        ...
                        else if (variables.MenuMode == RIS.RISMode.Advanced && prop.MaterialOverride == null)
                        {
        ...
        private static void BuildFXLayer(RISVariables variables, string autoGeneratedFolder)
        {
            ...
            foreach (var groupIndex in Enumerable.Range(0, variables.Groups.Count))
            {
                ...
                foreach (var propIndex in Enumerable.Range(0, group.Props.Count))
                {
                    ...
                    var layerName = $"RIS-MAIN-G{groupIndex}P{propIndex}";

                    var paramName = $"RIS-G{groupIndex}P{propIndex}";
                        ...
                        if (group.ExclusiveMode == 2 || group.ExclusiveMode == 3)
                        {
                            ...
                            foreach (var subPropIndex in Enumerable.Range(0, group.Props.Count))
                            {
                                if (propIndex != subPropIndex)
                                    ...
                                    {
                                        name = $"RIS-G{groupIndex}P{subPropIndex}",
                    ...
                    else if (variables.MenuMode == RIS.RISMode.Advanced)
                    {
                        ...
                        if (prop.MaterialOverride == null)
                        {
                            ...
                            {
                                var additionalLayerName = $"RIS-ANIM-G{groupIndex}P{propIndex}";
                            ...
                            if (prop.PropGroupType != RIS.PropGroup.None && variables.AdvancedGroupMode[(int)prop.PropGroupType] == 1)
                            {
                            ...
                            if (prop.PropGroupType != RIS.PropGroup.None)
                            {
                                var driver = onState.AddStateMachineBehaviour<VRCAvatarParameterDriver>();
                                foreach (var subGroupIndex in Enumerable.Range(0, variables.Groups.Count))
                                {
                                    ...
                                    foreach (var subPropIndex in Enumerable.Range(0, subGroup.Props.Count))
                                    {
                                        ...
                                        if (!(groupIndex == subGroupIndex && propIndex == subPropIndex) && subProp.PropGroupType == prop.PropGroupType)
                                        {
                                            driver.parameters.Add(new VRC_AvatarParameterDriver.Parameter()
                                            {
                                                name = $"RIS-G{subGroupIndex}P{subPropIndex}",
                                                type = VRC_AvatarParameterDriver.ChangeType.Set,
                                                value = 0
                                            });
                                        }
                        ...
                        {
                            var timerLayerName = $"RIS-TIMER-G{groupIndex}P{propIndex}";
                            ...
                            driver.parameters.Add(new VRC_AvatarParameterDriver.Parameter()
                            {
                                name = $"RIS-G{groupIndex}P{propIndex}",
                                type = VRC_AvatarParameterDriver.ChangeType.Set,
                ...
                {
                    var layerName = $"RIS-RESET-G{groupIndex}";

                    var paramName = $"RIS-G{groupIndex}RESET";
                    ...
                    driver.parameters =
                        ...
                        {
                            name = $"RIS-G{groupIndex}P{num}",
                            type = VRC_AvatarParameterDriver.ChangeType.Set,
                            value = group.Props[num].IsDefaultEnabled ? 1 : 0
                        }).ToList();
                    driver.parameters.Add(new VRC_AvatarParameterDriver.Parameter()
                    {
                        name = $"RIS-G{groupIndex}RESET",
                        type = VRC_AvatarParameterDriver.ChangeType.Set,
                        value = 0
                    });
                }
            ...
            if(variables.MenuMode == RIS.RISMode.Advanced && materialOverrideDatas.Count > 0)
            {
                    ...
                    if (renderer != null)
                    {
                        ...
                        if (baseMaterial != null)
                        {
                            ...
                            foreach (var materialIndex in Enumerable.Range(0, materials.Count))
                            {
                                ...
                                var paramName = $"RIS-G{materialReference.index.group}P{materialReference.index.prop}";
                                CheckParam(avatar, fxLayer, paramName, prop.IsDefaultEnabled);
                                if (prop.LocalOnly)
                                    CheckParam(avatar, fxLayer, "IsLocal", false);
                                    ...
                                    foreach (var subGroupIndex in Enumerable.Range(0, variables.Groups.Count))
                                    {
                                        ...
                                        foreach (var subPropIndex in Enumerable.Range(0, subGroup.Props.Count))
                                        {
                                            ...
                                            if (!(groupIndex == subGroupIndex && propIndex == subPropIndex) && subProp.PropGroupType == prop.PropGroupType)
                                            {
                                                ...
                                                var paramKey = $"RIS-G{subGroupIndex}P{subPropIndex}";
                                ...
                                foreach (var subMaterialIndex in Enumerable.Range(0, materials.Count))
                                {
                                    if(subMaterialIndex != materialIndex)
                                    {
                                        ...
                                        var paramKey = $"RIS-G{groupIndex}P{propIndex}";
                                ...
                                var removeKey = $"RIS-G{materialReference.index.group}P{materialReference.index.prop}";
                            ...
                            foreach (var material in materials)
                            {
                                ...
                                offTransition.AddCondition(AnimatorConditionMode.IfNot, $"RIS-G{material.index.group}P{material.index.prop}", 0f, prop.LocalOnly && !prop.IsDefaultEnabled, true);
                            }
                            EditorUtility.SetDirty(clipOFF);
                            layer.stateMachine = stateMachine;
                            EditorUtility.SetDirty(layer.stateMachine);
            ...
            foreach(var pairsIndex in Enumerable.Range(0,exclusiveGroupIndexes.Length))
            {
                ...
                var layerName = $"RIS-PAIR{pairsIndex}";
                ...
                foreach(var v in Enumerable.Range(0, pairs.Count + 1))
                {
                    ...
                    {
                        var targetPair = pairs[activeIndex];
                        var paramName = $"RIS-G{targetPair.group}P{targetPair.prop}";
                        CheckParam(avatar, fxLayer, paramName, false);
                        transition = stateMachine.MakeAnyStateTransition(state);
        ...
        private static void BuildExpressionParameters(RISVariables variables, string autoGeneratedFolder)
        {
            ...
            foreach (var groupIndex in Enumerable.Range(0, variables.Groups.Count))
            {
                ...
                if ((variables.MenuMode == RIS.RISMode.Simple && group.ExclusiveMode == 1) ||
                    (variables.MenuMode == RIS.RISMode.Advanced && group.UseResetButton))
                    TryAddParam(variables, $"RIS-G{groupIndex}RESET", 0f, false);
                foreach (var propIndex in Enumerable.Range(0, group.Props.Count))
                {
                    var prop = group.Props[propIndex];
                    var v2Mode = false;
                    if ((variables.MenuMode == RIS.RISMode.Simple && group.ExclusiveMode == 3) ||
                        (variables.MenuMode == RIS.RISMode.Advanced && prop.PropGroupType != RIS.PropGroup.None && variables.AdvancedGroupMode[(int)prop.PropGroupType] == 1))
                        v2Mode = true;
                    TryAddParam(variables, $"RIS-G{groupIndex}P{propIndex}", prop.IsDefaultEnabled && !v2Mode ? 1f : 0f, prop.SaveParameter);
                }
            }
